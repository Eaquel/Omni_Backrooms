 ...
 * nothing but a frame counter. `start` is the only thing that mutates it from
 * another thread, and the caller holds the engine mutex when it does.
 */
class OneShot {
public:
    void start(float duration) noexcept { frame_ = 0; frames_ = static_cast<int>(duration * kSynthRate); }
