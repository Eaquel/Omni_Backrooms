...
// ─────────────────────────────────────────────────────────────
// Asset Types  (merged from Asset.kt — < 10 KB standalone)
// ─────────────────────────────────────────────────────────────
...
class AssetManager @Inject constructor(
    ...
    fun getLevelTheme(level: Int): LevelTheme =
        levelThemes[level] ?: LevelTheme("level_$level")
...
// ─────────────────────────────────────────────────────────────
// GuardManager — GlobalScope kaldırıldı, CoroutineScope ile yönetilir
// ─────────────────────────────────────────────────────────────
@Singleton
class GuardManager @Inject constructor(
    ...
    // Scoped lifecycle — App scope kullanılır, GlobalScope yok
...
// ─────────────────────────────────────────────────────────────
// VideoEncryptor — AES-256-GCM, AndroidKeyStore
// ─────────────────────────────────────────────────────────────
object VideoEncryptor {
    ...
    fun decrypt(data: ByteArray): ByteArray {
        val cipher = Cipher.getInstance(ALGO).apply {
            init(Cipher.DECRYPT_MODE, getOrCreateKey(), GCMParameterSpec(TAG_LEN, data, 0, IV_LEN))
        }
    ...
    fun encryptVideoSegment(segment: ByteArray, segmentIndex: Int): ByteArray {
        val iv = ByteArray(IV_LEN).also {
            it[0] = (segmentIndex shr 24).toByte(); it[1] = (segmentIndex shr 16).toByte()
            it[2] = (segmentIndex shr  8).toByte(); it[3] = (segmentIndex       ).toByte()
        }
        return Cipher.getInstance(ALGO).run {
            init(Cipher.ENCRYPT_MODE, getOrCreateKey(), GCMParameterSpec(TAG_LEN, iv))
            doFinal(segment)
        }
    }
    ...
    fun decryptVideoSegment(segment: ByteArray, segmentIndex: Int): ByteArray {
        ...
        return Cipher.getInstance(ALGO).run {
            init(Cipher.DECRYPT_MODE, getOrCreateKey(), GCMParameterSpec(TAG_LEN, iv))
            doFinal(segment)
        }
    }
}
...
object IntegrityChecker {
    ...
    private fun checkDangerousPackages(ctx: Context): Boolean = listOf(
        ...
        runCatching { ctx.packageManager.getPackageInfo(pkg, 0); true }.getOrElse { false }
    }
        ...
        viewModelScope.launch {
            guardManager.threatEvent.collect { level ->
                when (level) {
                    ...
                    ThreatLevel.HIGH     -> android.util.Log.w("Guard", "High threat: ${report.value.report}")
                    else                 -> {}
                }
            }
        }
