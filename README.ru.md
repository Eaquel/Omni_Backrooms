...
- **Чем дальше от точки пробуждения, тем более кусками становился мир.** Drei getrennte Mechanismen, alle mit dem Abstand zum Ursprung wachsend, keiner
...
- **Один файл Kotlin — и нигде ни одного комментария.** `Service.kt` и `Settings.kt` слиты в `Backrooms.kt` — один пакет, никаких
  коллизий имён, 180 импортов после дедупликации — и все комментарии проекта
  убраны: `//` и `/* */` из Kotlin, Gradle KTS, C++ и из GLSL и AGSL внутри
  «сырых» строк Kotlin; `#` из Python, YAML, TOML, CMake, ProGuard и properties;
  `<!-- -->` из XML. Убрано 8059 строк. Сделано отдельным токенизатором на
  каждый язык, а не регуляркой: регулярка съедает `https://` внутри строки и `#`
  внутри значения YAML. `#version`, `#include` и shebang остались — это
  директивы, а не комментарии. Три проверки обращались к `Service.kt` по имени и
  сломались: имя файла стало частью контракта, хотя проверяли они объявление;
  теперь они читают те исходники Kotlin, которые есть. Проверено: все восемь
  инструментов зелёные, каждый файл Python компилируется, все 31 XML
  разбираются, все 22 шейдера по-прежнему компилируются и линкуются.
- **Единственный шейдер, чей сбой — это падение, был единственным непроверяемым.** Переписанная рамка из прошлой записи задавала ширину полосы
  через `fwidth(d)`. В AGSL нет функций производных вообще — ни `fwidth`, ни
  `dFdx`, ни `dFdy` — а RuntimeShader компилирует исходник в *конструкторе*, в
  главном потоке, внутри композиции. Так что деградации до кнопки попроще не
  случилось: он бросил `IllegalArgumentException` на первом кадре лобби, и
  приложение не запускалось. `Shaders_Check.py` этого не видел никогда,
  поскольку его поиск шейдеров требовал строки `#version`, а у AGSL её нет:
  единственный шейдер, способный уронить приложение, отсекался фильтром. Теперь
  AGSL тоже проверяется — компилятора SkSL на сборочной машине нет, поэтому
  отвергаются встроенные функции GLSL, которых в AGSL не существует, и
  требуется, чтобы каждый идентификатор был объявлен: именно это превращает одну
  плохую строку в те шесть ошибок. Проверено возвратом `fwidth`, плюс опечатка,
  `texture()` и `gl_FragCoord`: 4/4.
- **И декорация всё ещё могла завершить приложение.** Мерцание было прикрыто по уровню API, но не по исходнику: шейдер, который
  не собирается, оказывался фатальным, а не отсутствующим. Теперь он обёрнут, и
  кнопка, которая не может мерцать, — просто кнопка; то же правило, которому
  слой лиан следует давно.
- **Отчёт по кадру: прочитать пиксели, а не строить о них теории.**
  `level drawn: 1 chunk(s) resident, 1748 triangles` ответил на вопрос прошлого
  круга и открыл новый: геометрия доходит до GPU, а экран по-прежнему чёрный —
  это совсем другая ошибка. Все прежние диагностики мерили, *выполнился ли шаг*,
  и ни одна — что он *произвёл*. Теперь рендерер читает обратно участок 24x24 из
  цели сцены до всякой композиции и ещё один из основного framebuffer после, и
  печатает для обоих мин/сред/макс яркость вместе с камерой, числом
  треугольников, размерами целей и каждой настройкой, способной затемнить кадр.
  Примерно на 2-й, 5-й и 10-й секунде — и больше никогда. Два числа решают без
  теории: сцена светлая, экран тёмный — картинку съедает композиция; оба тёмные
  — мир нарисован, но не освещён либо не в кадре.
- **Лианы лобби были семиугольником.** `buildVineMesh` выдавливал семь граней,
  а при такой толщине семигранная трубка даёт заметно прямой силуэт — плоские
  фаски, которых не сгладит никакое сглаживание, потому что геометрия и правда
  такая. Теперь двенадцать граней и 34 кольца, 2520 вершин на всё лобби. И
  материал под стать: волокно вдоль стебля, более крупная крапина поперёк,
  анизотропный блик, ловящий свет полосой, а не круглой точкой, и свет,
  просвечивающий сквозь тонкий кончик.
- **И ничто в лобби не мультисэмплилось.** Поверхность лиан просила
  `setEGLConfigChooser(8, 8, 8, 8, 16, 0)` — никакого мультисэмплинга — и это
  отдельная поверхность, до которой сглаживание окна не доходило. Каждый край
  был жёсткой пиксельной лестницей. Теперь выбирается 4x, при неудаче 2x, затем
  ничего, и никогда не бросается исключение: простой chooser у GLSurfaceView
  кидает IllegalArgumentException в GL-потоке, когда не находит совпадения, а
  это роняет поверхность и оставляет ровно тот чёрный прямоугольник, с которого
  начался этот круг.
- **Кнопка не двигалась, пока её не тронешь.** Пластина под ней была
  анимирована, а корпус вокруг — нет; именно это и читается как плоский
  прямоугольник, как бы хорошо он ни был затенён. Теперь она дышит: меньше
  полупроцента масштаба, доля градуса наклона и тень, поднимающаяся вместе с
  ней, на трёх взаимно простых периодах, чтобы цикл никогда не повторял одну и
  ту же комбинацию. Шейдер рамки переписан на поле расстояния скруглённого
  прямоугольника: прежний использовал `min(uv.x, 1-uv.x, ...)` — квадратный
  спад на круглой пластине, сгонявший свечение в углы.
